...
class ShmemVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, spaces=None, context="spawn"):
        ...
        raw_obs_buf = {
            k: alloc_shmem(
                ctx, (len(env_fns),) + self.obs_shapes[k], self.obs_dtypes[k].type
            )
            for k in self.obs_keys
        }
        self.obs_buf = {
            k: np.frombuffer(raw_obs_buf[k], dtype=self.obs_dtypes[k].type).reshape(
                (len(env_fns),) + self.obs_shapes[k]
            )
            for k in self.obs_keys
        }
        raw_act_buf = alloc_shmem(
            ctx, (len(env_fns),) + action_space.shape, action_space.dtype.type
        )
        self.act_buf = np.frombuffer(
            raw_act_buf, dtype=action_space.dtype.type
        ).reshape((len(env_fns),))
        raw_rew_buf = alloc_shmem(ctx, (len(env_fns),), np.float32)
        self.rew_buf = np.frombuffer(raw_rew_buf, dtype=np.float32).reshape(
            (len(env_fns),)
        )
        raw_done_buf = alloc_shmem(ctx, (len(env_fns),), np.bool_)
        self.done_buf = np.frombuffer(raw_done_buf, dtype=np.bool_).reshape(
            (len(env_fns),)
        )
        self.parent_pipes = []
        self.procs = []
    ...
    def step_wait(self):
        ...
        return (
            self._decode_obses(),
            np.copy(self.rew_buf),
            np.copy(self.done_buf),
            infos,
        )
    ...
    def get_action_influence(self):
        for pipe in self.parent_pipes:
            pipe.send("get_action_influence")
...
def _subproc_worker(
    ...
    """
    obs_buf = {
        k: np.frombuffer(raw_obs_buf[k], dtype=obs_dtypes[k].type).reshape(
            (-1,) + obs_shapes[k]
        )
        for k in obs_keys
    }
        ...
        while True:
            ...
            if cmd == "reset":
                ...
                if done:
                    ...
                    info = {
                        'action_influence': env.get_action_influence(),
                        'truncated': False
                    }
                else:
                ...
                pipe.send(None)
                break
            elif cmd == "get_action_influence":
                pipe.send(env.get_action_influence())
            else:
